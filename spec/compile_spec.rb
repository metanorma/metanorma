...
RSpec.describe Metanorma::Compile do
  ...
  it "fontist_install called" do
    ...
    allow(Metanorma::Util::FontistUtils).to receive(:fontist_install) {}
    expect(Metanorma::Util::FontistUtils).to receive(:fontist_install).once
  ...
  it "skip font install with no_install_fonts" do
    ...
    allow(Metanorma::Util::FontistUtils).to receive(:fontist_install) {}
    expect(Metanorma::Util::FontistUtils).not_to receive(:fontist_install)
  ...
  it "handle missing fontist index" do
    ...
    @called = 0
    allow(Metanorma::Util::FontistUtils).to receive(:fontist_install) do
      @called += 1
      raise Fontist::Errors::FormulaIndexNotFoundError if @called == 1
    end
    allow(Fontist::Formula).to receive(:update_formulas_repo)
    expect(Metanorma::Util::FontistUtils).to receive(:fontist_install).twice
  ...
  it "handle no_progress" do
    ...
    expect(Metanorma::Util::FontistUtils).to receive(:fontist_install)
  ...
  it "handle :fonts attribute in adoc" do
    ...
    allow(Metanorma::Util::FontistUtils).to receive(:install_fonts_safe)
      .with(hash_including("MS Gothic"), false, false, false)
      .and_return(nil)

    allow(Metanorma::Util::FontistUtils).to receive(:location_manifest)
